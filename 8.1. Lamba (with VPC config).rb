...
class Stack < Halloumi::CompoundResource
  module Concerns
    module ExampleLambdaWithVpc #:nodoc:
      ...
      included do
        ...
        resource :example_lambda_subnet_groups,
          ...
          r.property(:amount) { 3 }
          r.property(:service_ip_offset) { 6 }
          r.resource(:skeletons) { skeletons }
        end

        resource :example_lambda_security_groups,
                 type: Halloumi::AWS::EC2::SecurityGroup do |r|
          r.property(:group_description) { "Example Lambda SecurityGroup" }
          r.property(:security_group_egress) do
            [
              {
                IpProtocol: "tcp",
                FromPort: "-1",
          ...
          r.property(:tags) do
            [
              {
                Key: :Name,
                Value: example_lambda_security_group._id
              }
            ]
          end
        ...
        resource :example_lambda_functions,
          ...
          r.property(:vpc_config)  do
            {
              SecurityGroupIds: [ example_lambda_security_group.ref ],
              SubnetIds: example_lambda_subnet_group.subnets.map(&:ref)
            }
          end
        end
